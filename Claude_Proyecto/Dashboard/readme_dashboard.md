...
Nota: Skincare, Cuidado de la Salud y Ejercicio viven ahora dentro de `CuidadoPersonal/cuidadopersonal.html` (subtabs), que a su vez incrusta `salud.html` y `ejercicio.html` completos vía `<iframe>`. El Dashboard lee `mirutina_v1` desde el 2026-07-30 (`D.gym`, solo `sesiones` y `metas.frecuencia`, para el slide Hero) — sigue sin leer nada más de esa app (ni progreso de fuerza ni qué rutina toca hoy).
...
- **🌟 Hero** (slide 1) — **rediseñado el 2026-07-30** ("eso no me aporta nada", sobre la versión anterior de "Vida Score"): ya no calcula un score compuesto abstracto (`calcScores()`/`vidaScore()`/`AREAS`/`sCol()`/`sLab()` — eliminados por completo, no queda nada de esa lógica). Ahora es "📊 Tu semana en números" (`renderHero()`), 3 métricas reales de la semana calendario actual (`semanaActual()`, Domingo→Sábado, misma convención que `ejercicio.html`): **🏋️ Gym esta semana** (sesiones en `D.gym.sesiones` vs. `D.gym.metas.frecuencia`), **🥗 Nutrición registrada** (días distintos con algún alimento en `D.sal.alimentos`, sobre 7), **✅ Tareas completadas** (tasa general de `D.pro.tareas`, no "de esta semana" — Proyectos no guarda fecha de completado, así que filtrar por semana sería un dato inventado). Cada tile cambia de color (verde/amarillo/rojo) según qué tan cerca esté de la meta. El reloj se mantiene arriba pero mucho más chico (antes dominaba la pantalla con 120px; ahora es secundario, `clamp(34px,4.2vw,58px)`).
