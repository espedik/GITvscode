...
- **🧠 Habilidades** (slide 4) — layout asimétrico `grid-template-columns:0.8fr 1.35fr`: columna izquierda angosta con el radar reducido (`.chart-wrap-sm`, `clamp(150px,18vh,220px)` — antes `.chart-wrap` normal, `clamp(240px,34vh,420px)`) y el OVR general chico debajo; columna derecha ancha con `#skillPriority` (`renderSkills()`), el elemento con más peso visual del slide (título propio `.skill-priority-title`, 14-18px, más grande que cualquier `.lbl`). Muestra las **`PRIORIDAD_N` (4) prioridades más bajas** de `APRENDIZAJE` (de las 5 totales — `.slice(0,PRIORIDAD_N)` tras ordenar por `val` ascendente, sube/baja sola si cambian los valores del radar), cada una como una fila `.skill-row` de **una sola línea con letra chica** (nombre 10.5px, valor y resumen ~9.5px — clases `.sr-name`/`.sr-val`/`.sr-primer`): ícono, nombre, valor y un resumen accionable corto (`a.corto`, campo nuevo en `APRENDIZAJE`, distinto del `a.primer` largo que usa Coach), siempre visible, sin clics. **Iteración 2026-07-30** (3 rondas de feedback de Adán): 1) subtabs clicables → mala idea, el slide rota solo, nadie hace clic; 2) 5 prioridades con texto largo → mucho para un vistazo; 3) "compacto" se entendió mal como "menos ítems" → el pedido real era letra más chica + radar más chico + la sección de prioridades más grande, que es el layout actual. Link final a Coach → Aprendizaje para las 5 completas con hábito + los 4 recursos de cada una.
