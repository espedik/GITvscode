...
- **🌟 Hero** (slide 1) — **"JARVIS · Tu semana completa"**. Tercer rediseño el 2026-07-30, cada vez con más detalle real (ver historial abajo). Reloj+fecha ahora chicos y a la derecha (`clamp(20px,2.2vw,28px)`, ya no dominan la pantalla). Debajo, dos bloques:
  1. **Tira de 7 días** (`#heroWeekStrip`, `.week-strip`, Domingo→Sábado vía `semanaActual()`) — una tarjeta `.ws-day` por día con: nombre corto + fecha, ícono de gym (✅ si hay sesión real ese día en `D.gym.sesiones`, 😴 si la rutina de ese día es descanso, ⏳ si es hoy/futuro y todavía no se entrena, ❌ si ya pasó y no se entrenó) + el **nombre real de la rutina** de ese día (`s.nombre` de la sesión, o `D.gym.rutina[dow].nombre` si aún no se hizo), y las calorías registradas ese día (`D.sal.alimentos` sumado por fecha). El día de hoy se resalta con borde de color.
  2. **3 paneles de detalle** (`.grid.g3`, `.tile` con clase `.hp-*`), con datos reales y específicos, no agregados abstractos:
     ...
     - **🥗 Nutrición de hoy** (`#heroNutriPanel`) — calorías de hoy vs. meta en grande, más 3 barras de progreso (proteína/carbs/grasa) con los valores reales de `D.sal.alimentos` de hoy contra `D.sal.metas`.
     - **✅ Tareas pendientes** (`#heroTareasPanel`) — hasta 5 tareas reales de `D.pro.tareas` (campos `titulo`/`fecha_limite`/`estado`, mismos que usaba el `getAlerts()` original) ordenadas por fecha límite, en rojo si vencidas y en amarillo si vencen hoy.
