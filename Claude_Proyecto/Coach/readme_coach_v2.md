...
- El **Dashboard** (`../Dashboard/dashboard.html`) lee `coach_rutina_v1` directamente (`D.rut`) y **duplica** `RUTINA_TASKS` (54 tareas de nivel superior / 85 contando subtareas, deben quedar byte-idénticas — verificado con `JSON.stringify` en cada cambio), las 4 fechas de `PHASES` junto con el desglose semana a semana de cada `<ul>` de fase (`PHASES[].semanas`, texto plano sin los `onclick` de `irANegocios()` — el Dashboard pasó a mostrar este detalle completo el 2026-08-02, antes mostraba solo 3 prioridades resumidas), y el array `SK` de 12 skills. **Si editas el horario de `#rutina`, los valores base del radar, o las fechas/semanas del Plan Maestro aquí, hay que replicar el cambio en `Dashboard/dashboard.html`** — no hay sincronización automática entre archivos. Ver tabla de "Datos duplicados" en [`../README.md`](../README.md).
