...
- **🏋️ Mi Rutina** — sección fusionada (antes Dashboard + Mi Rutina eran dos secciones separadas): banner de meta activa, 4 KPIs calculados directo del plan semanal (días de entreno, días de descanso, grupos musculares cubiertos, qué toca hoy), vista de la semana completa como **tira horizontal de 7 días** (`.week-strip`/`.ws-day`, clic en cualquier día hace scroll hasta su detalle) y, debajo, **los 7 días completos uno tras otro** (`#rutina-semana`, `renderRutinaSemana()` — ver "'Mi Rutina' pasa de un día a la vez a los 7 días completos" del 2026-08-04 más abajo; ya no hay subtabs de "un día a la vez"), cada uno con su tarjeta grande `.exd-card` por ejercicio — imagen (cuando existe), número, músculo/equipo, series×reps×descanso, y la técnica (`cue`) en un recuadro destacado. Ver "Programa por defecto" abajo.
...
| Miércoles | 🔙 Espalda + Hombros + Cardio HIIT (Battle Ropes) | También construye brazos (secundarios en dominadas/remo/press) + cardio para bajar grasa |
