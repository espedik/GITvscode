...
- El **Dashboard** (`../Dashboard/dashboard.html`) lee `mirutina_v1` desde el 2026-07-30 (`D.gym` en `loadAll()`), pero **solo** `sesiones` y `metas.frecuencia` — se usa en el slide Hero ("📊 Tu semana en números", tile "🏋️ Gym esta semana": sesiones de la semana calendario actual vs. la meta). No lee `S.rutina` (qué toca hoy) ni el detalle de series/pesos — la píldora de acceso directo en la barra superior fija (`renderQuickApps()`) tampoco muestra estadística de esta app. Si se quiere más integración (racha, próxima rutina, progreso de fuerza), es trabajo pendiente.
