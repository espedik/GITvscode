// Genera Aeroresinas-dossier.pdf a partir de dossier.html.
//
//   node generar-pdf.js          → español (Aeroresinas-dossier.pdf)
//   node generar-pdf.js en       → inglés  (Aeroresinas-dossier-en.pdf)
//
...
const SALIDA = path.join(__dirname, EN ? 'Aeroresinas-dossier-en.pdf' : 'Aeroresinas-dossier.pdf');
  ...
  const rev = await pg.evaluate(() => {
    ...
    document.querySelectorAll('.pg').forEach((p, i) => {
      ...
      p.querySelectorAll('*').forEach(el => {
        ...
        if (el.closest('.pie-pg, .portada-foto, .portada-pie')) return;
